        ...
        for x, z in circular_coordinates_by_count_gen(radius=self._radius, count=count):
            ...
            if obj is not None:
                ...
                obj.move_to(
                    ...
                    routing_surface=source.routing_surface,
                )
...
class SpawnObjectLoot(BaseLootOperation):
    FACTORY_TUNABLES = {
        ...
        "owner": TunableVariant(
            ...
            inherit=InheritOwnership.TunableFactory(),
        ),
    }
    ...
    def _apply_to_subject_and_target(self, subject, target, resolver):
        ...
        if obj is not None:
            obj.opacity = 0
            x_jitter = (
                random.uniform(self._x_jitter.lower_bound, self._x_jitter.upper_bound)
                if self._x_jitter is not None
                else 0
            )
            z_jitter = (
                random.uniform(self._z_jitter.lower_bound, self._z_jitter.upper_bound)
                if self._z_jitter is not None
                else 0
            )
            position = sims4.math.Vector3(0, 0, 0)
