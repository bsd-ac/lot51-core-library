...
class _PortalTypeDataAdvancedTeleport(RouteEventProviderMixin, _PortalTypeDataTeleport):
    ...
    def get_portal_locations(self, obj):
        ...
        for connected_object in object_manager.get_objects_with_tags_gen(
            *self.destination_object_tags
            ...
            if connected_object is not obj and self.destination_object_tests.run_tests(
                resolver
            ):
    ...
    def provide_route_events(
        self, route_event_context, sim, path, is_mirrored=True, node=None, **kwargs
    ):
        for route_event_data in self.route_events:
            if route_event_context.route_event_already_scheduled(
                route_event_data.route_event, provider=self
            ) or not route_event_context.route_event_already_fully_considered(
                route_event_data.route_event, self
            ):
                route_event_context.add_route_event(
                    RouteEventType.HIGH_SINGLE,
                    route_event_data.route_event(
                        provider=self, time=node.time + route_event_data.offset
                    ),
                )
