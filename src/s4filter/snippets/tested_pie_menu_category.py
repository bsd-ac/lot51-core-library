    ...
    def get_category(cls, affordance, resolver, from_inventory_to_owner=False):
        for snippet in cls.all_snippets_gen():
            for row in snippet.categories:
                ...
                if (
                    row.basic_compatibility == PieMenuCategoryCompatibility.TARGET_ONLY
                    and from_inventory_to_owner
                ) or (
                    row.basic_compatibility
                    == PieMenuCategoryCompatibility.FORWARDED_ONLY
                    and not from_inventory_to_owner
                ):
                    continue
...
def _edit_choice_menu_categories(original, self, aop, context, result):
    ...
    try:
        ...
        from_inventory_to_owner = (
            aop.interaction_parameters is not None
            and aop.interaction_parameters.get("from_inventory_to_owner", False)
        )
