...
class ModManifest(
    ...
    INSTANCE_TUNABLES = {
        ...
        "version": Tunable(
            tunable_type=str,
            default="1.0.0",
            description="A semver version pattern that represents the current package version.",
        ),
    ...
    def _build_version_tns(cls, script_ver, package_ver):
        ...
        if cls.version_mismatch_notification.http is not None:
            ...
            arg0 = DialogHelper.create_arg(
                CommandArgType.ARG_TYPE_STRING,
                cls.version_mismatch_notification.http.base_url,
            )
            arg1 = DialogHelper.create_arg(
                CommandArgType.ARG_TYPE_STRING,
            ...
            ui_responses.append(
                DialogHelper.build_ui_response(
                    response_id=DialogHelper.ButtonType.DIALOG_RESPONSE_OK,
                    text=ui_button_text,
        ...
        dialog = DialogHelper.create_notification(
            title=title,
            text=text,
            urgency=DialogHelper.UiDialogNotificationUrgency.URGENT,
            expand_behavior=DialogHelper.UiDialogNotificationExpandBehavior.FORCE_EXPAND,
