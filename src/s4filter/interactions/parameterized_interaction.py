...
class ParameterizedRequestContinuationMixin:
    ...
    def _run_paramaterized_request(self, obj):
        ...
        if obj_available:
            ctx = self.context.clone_for_continuation(
                self,
                source=parameters.context_source_override
                if parameters.context_source_override is not None
                else self.context.source,
            ...
            for aop in obj.potential_interactions(ctx):
                try:
                    if ctx.source == InteractionContext.SOURCE_AUTONOMY:
                        if (
                            not aop.affordance.allow_autonomous
                            or not aop.affordance.test_autonomous.run_tests(
                                resolver,
                    ...
                    for sc_data in interaction._static_commodities:
                        if (
                            sc_data.static_commodity in static_commodity_list
                            and sc_data.static_commodity
                            not in exclude_static_commodity_list
                        ):
                            total_desire += (
                                sc_data.desire * sc_data.static_commodity.ad_data
                            )
                            ...
                            if stat_op_list.is_valid(interaction):
                                ...
                                if stat is not None:
                                    ...
                                    if stat.stat_type in parameters.commodities:
                                        ...
                                        scored_value = (
                                            stat.autonomous_desire
                                            * actor.get_score_multiplier(stat.stat_type)
                                            * stat.autonomy_weight
                                            * fulfillment_rate
                                        )
                                        logger.debug(
    ...
    def test(cls, inst, context=DEFAULT, **kwargs):
        ...
        try:
            ...
            return TestResult(
                False,
                "No objects found",
                tooltip=inst_or_cls.object_source_fail_tooltip,
            )
        except:
