...
class TunablePurchaseItem(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {
        "stock_id": Tunable(
            description="Unique Stock ID override, by default will set a UUID",
            tunable_type=str,
            default=None,
            allow_empty=True,
        ),
        ...
        "hide_if_sold_out": Tunable(
            description="Hides from picker if stock level is 0 instead of showing sold out text/icon",
            tunable_type=bool,
            default=False,
        ),
        "include_build_buy_tags": Tunable(
            description="Toggle the objects catalog tags to be included in the category tags",
            tunable_type=bool,
            default=False,
        ),
        "include_recipe_tags": Tunable(
            description="Toggle the recipe tags to be included in the category tags",
            tunable_type=bool,
            default=False,
        ),
        ...
        "set_custom_name_on_object": Tunable(
            description="If custom_names is enabled, the chosen name will be set in the NAME_COMPONENT and provided as the 0 index token in description. Set to false to only use the custom name for the description.",
            tunable_type=bool,
            default=True,
        ),
        "set_sim_as_owner": Tunable(tunable_type=bool, default=False),
        "show_discount": Tunable(tunable_type=bool, default=False),
        "use_base_cost_as_value": Tunable(
            tunable_type=bool,
            default=False,
