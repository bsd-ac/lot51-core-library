        ...
        for situation_type in self.situations:
            if self.compatible_venues is not None:
                inject_list(
                    situation_type,
                    "compatible_venues",
                    new_items=self.compatible_venues,
                )

            if self.disallows_curfew_violation is not None:
                situation_type.disallows_curfew_violation = (
                    self.disallows_curfew_violation
                )
            ...
            if self.situation_display_type_override is not None:
                situation_type.situation_display_type_override = (
                    self.situation_display_type_override
                )
            ...
            if (
                self.additional_activity_selection is not None
                and situation_type.activity_selection is not None
            ):
                new_activity_selection = AttributeDict()
                if len(self.additional_activity_selection.available_activities):
                    new_activity_selection.available_activities = merge_list(
                        situation_type.activity_selection.available_activities,
                        self.additional_activity_selection.available_activities,
                    )

                if self.additional_activity_selection.required_activities is not None:
                    if (
                        situation_type.activity_selection.required_activities
                        is not None
                    ):
                        new_activity_selection.required_activities = merge_list(
                            situation_type.activity_selection.required_activities,
                            self.additional_activity_selection.required_activities,
                        )
