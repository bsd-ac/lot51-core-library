...
from interactions.utils.display_name import (
    TunableDisplayNameVariant,
    TunableDisplayNameWrapper,
)
...
from lot51_core.tunables.crafting_interaction_injection import (
    TunableCraftingInteractionInjection,
)
from lot51_core.tunables.map_view_picker_injection import (
    TunableMapViewPickerInteractionInjection,
)
from lot51_core.tunables.purchase_interaction_injection import (
    TunablePurchaseInteractionInjection,
)
    ...
    FACTORY_TUNABLES = {
        ...
        "interaction_category_tags": TunableSet(
            ...
            tunable=TunableEnumEntry(
                description="These tag values are used for searching, testing, and categorizing interactions.",
                tunable_type=Tag,
                default=Tag.INVALID,
                pack_safe=True,
            ),
            ...
            tunable=TunableTuple(
                ...
                posture_outfit_change_overrides=OptionalTunable(
                    tunable=TunableMapping(
                        ...
                        key_type=TunableReference(
                            description="If the Sim encounters this posture during this interaction's transition sequence, the posture's outfit change reasons will be the ones specified here.",
                            manager=services.get_instance_manager(Types.POSTURE),
                        ),
    ...
    def _inject_display_name_wrappers(self, affordance):
        if affordance.display_name_wrappers is None:
            ...
            affordance.display_name_wrappers = clone_factory_with_overrides(
                ...
                wrappers=merge_list(
                    affordance.display_name_wrappers.wrappers,
                    self.display_name_wrappers.wrappers,
                    prepend=True,
                ),
            )
    ...
    def _inject_display_name_overrides(self, affordance):
        if affordance.display_name_overrides is None:
            ...
            affordance.display_name_overrides = clone_factory_with_overrides(
                ...
                overrides=merge_list(
                    affordance.display_name_overrides.overrides,
    ...
    def inject(self):
        for affordance in self.get_affordances_gen():
            ...
            if self.allow_forward_from_object_inventory_override is not None:
                affordance.allow_forward_from_object_inventory = (
                    self.allow_forward_from_object_inventory_override
                )
            ...
            if self.allow_from_sim_inventory_override is not None:
                affordance.allow_from_sim_inventory = (
                    self.allow_from_sim_inventory_override
                )
            ...
            if self.interaction_category_tags is not None:
                inject_list(
                    affordance,
                    "interaction_category_tags",
                    self.interaction_category_tags,
                )
            ...
            if self.provided_template_affordances is not None:
                if affordance.provided_template_affordances is None:
                    ...
                    merged_template_affordances = merge_list(
                        affordance.provided_template_affordances.template_affordances,
                        self.provided_template_affordances.template_affordances,
                    )
                    affordance.provided_template_affordances = merge_dict(
                        affordance.provided_template_affordances,
                        template_affordances=merged_template_affordances,
                    )
        ...
        if self.utility is not None:
            for affordance in services.get_instance_manager(
                ...
                if affordance.utility_info is not None:
                    if (
                        self.utility in affordance.utility_info
    ...
    def get_affordances_gen(self):
        ...
        for affordance in services.get_instance_manager(
            ...
            if (
                ...
                and affordance.category in self.pie_menu_categories
                and affordance not in self.exclude_affordances
            ):
