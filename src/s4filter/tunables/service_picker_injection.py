...
class TunableServicePickerInjection(BaseTunableInjection):
    ...
    FACTORY_TUNABLES = {
        ...
        "icon": TunableIconAllPacks(
            description="The icon to be displayed in 'Hire a Service' UI",
        ),
        "name": TunableLocalizedStringFactory(
            description="The name to be displayed for this NPC in the 'Hire a Service' UI.",
        ),
        "cost_string": TunableVariant(
            ...
            no_cost_string=TunableLocalizedStringFactory(
                description="The description to be used for this NPC in the if there isn't a cost associated with it",
            ),
        ...
        "tag_list": TunableList(
            description="Tags to be filtered by.",
            tunable=TunableEnumEntry(tunable_type=Tag, default=Tag.INVALID),
            unique_entries=True,
        ),
        "tests": TunableGlobalTestSet(
            description="A set of global tests that are always run before other tests. All tests must pass in order for the interaction to run.",
        ),
    ...
    def inject(self):
        ...
        if picker_tuning is not None:
            inject_list(
                picker_tuning, "non_service_npcs", (self,), unique_entries=False,
            )
        else:
            logger.warn(
                f"Could not find service npc picker tuning to inject with id {self.picker_tuning}",
            )
...
class TunableHireableServicePickerInjection(BaseTunableInjection):
    ...
    FACTORY_TUNABLES = {
        ...
        "service_npc": TunableReference(
            manager=services.get_instance_manager(Types.SERVICE_NPC),
        ...
        "already_hired_tooltip": TunableLocalizedStringFactory(
            description="Tooltip that displays if the service has already been hired.",
        ),
        "tests": TunableGlobalTestSet(),
    }

    __slots__ = (
        "already_hired_tooltip",
        "service_npc",
        "tag_list",
        "tests",
    )
    ...
    def inject(self):
        ...
        if picker_tuning is not None:
            ...
            logger.warn(
                f"Could not find hireable service npc picker tuning to inject with id {self.picker_tuning}",
            )
