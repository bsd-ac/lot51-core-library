...
class TaggedRecipesSource(BaseDefinitionSource):
    ...
    def _get_definitions_gen(self, resolver=None):
        for def_data in self._get_definition_data_gen(resolver):
            yield def_data.definition
    ...
    def _get_definition_data_gen(self, resolver=None):
        ...
        for tag in self.tags:
            for recipe in get_recipes_matching_tag(tag):
                if (
                    hasattr(recipe, "has_final_product_definition")
                    and recipe.recipe_tags is not None
                    and recipe not in _yield_cache
                ):
                    if (
                        not set(recipe.recipe_tags).intersection(self.exclude_tags)
                        and recipe not in self.exclude_recipes
                    ):
    ...
    def _get_definitions_gen(self, resolver=None):
        for definition in services.definition_manager().get_definitions_for_tags_gen(
            ...
            if (
                ...
                and definition not in self.exclude_definitions
            ):
                yield definition
