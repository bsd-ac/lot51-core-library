...
class FglDeliveryMethod(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def __call__(self, resolver, obj, **kwargs):
        ...
        if isinstance(participant, TerrainPoint):
            translation, orientation, routing_surface = get_location_near_location(
                obj,
                participant.location,
                optimal_distance=self.optimal_distance,
                radius_width=self.radius_width,
                max_distance=self.max_distance,
                x_range=self.x_random_offset,
                z_range=self.z_random_offset,
            )
        elif isinstance(participant, SimInfo):
            sim = participant.get_sim_instance()
            translation, orientation, routing_surface = get_location_near_object(
                obj,
                sim,
                optimal_distance=self.optimal_distance,
                radius_width=self.radius_width,
...
class MailboxDeliveryMethod(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {
        "fallback_inventory": TunableEnumEntry(
            tunable_type=InventoryDeliveryFallback,
            default=InventoryDeliveryFallback.NONE,
        ),
    }
    ...
    def __call__(self, resolver, obj, participant=ParticipantType.Actor):
        ...
        if zone is not None:
            ...
            if (
                lot_hidden_inventory is not None
                and lot_hidden_inventory.player_try_add_object(obj)
            ):
                return True
...
class HouseholdInventoryDeliveryMethod(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {
        "household_inventory": TunableEnumEntry(
            tunable_type=InventoryDeliveryFallback,
            default=InventoryDeliveryFallback.ACTIVE_HOUSEHOLD,
        ),
    }
