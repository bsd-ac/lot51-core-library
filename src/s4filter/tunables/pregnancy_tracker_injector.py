            ...
            value_type=TunableTuple(
                trait_entries=TunableList(
                    tunable=TunableTuple(
                        chance=TunablePercent(default=100),
                        traits=TunableList(
                            tunable=TunableTuple(
    ...
    def inject(self):
        for origin, origin_modifiers in self.pregnancy_origin_modifiers.items():
            if origin in PregnancyTracker.PREGNANCY_ORIGIN_MODIFIERS:
                PregnancyTracker.PREGNANCY_ORIGIN_MODIFIERS[
                    origin
                ].trait_entries = merge_list(
                    PregnancyTracker.PREGNANCY_ORIGIN_MODIFIERS[origin].trait_entries,
                    origin_modifiers.trait_entries,
                )
            else:
