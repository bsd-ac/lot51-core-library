...
class TunablePurchaseInteractionInjection(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def inject_to_affordance(self, affordance):
        ...
        if (
            hasattr(affordance, "delivery_method")
            and self.delivery_method_override is not None
        ):
        ...
        if hasattr(affordance, "purchase_list_option"):
            inject_list(
                affordance,
                "purchase_list_option",
                self.purchase_list_option,
                unique_entries=False,
            )

        if (
            hasattr(affordance, "price_multiplier")
            and self.price_multiplier is not None
        ):
