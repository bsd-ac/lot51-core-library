...
class BaseTunableObjectInjection(BaseTunableInjection):
    FACTORY_TUNABLES = {
                ...
                default_value=TunableVariant(
                    ...
                    random=TunableList(
                        ...
                        tunable=TunableTuple(
                            ...
                            weight=Tunable(tunable_type=float, default=1.0),
                        ),
                    ),
                    default="reference",
                ),
        ...
        "timed_state_triggers": OptionalTunable(
                ...
                value_type=TunableTuple(
                    ...
                    ops=TunableList(
                        ...
                        tunable=TunableTuple(
                            ...
                            trigger_time_random_offset=TunableSimMinute(
        ...
        "inventory_item_component": OptionalTunable(
            tunable=TunableTuple(
                valid_inventory_types=TunableEnumSet(
                    enum_type=InventoryType,
                    enum_default=InventoryType.UNDEFINED,
        ...
        "routing_component": OptionalTunable(
            tunable=TunableTuple(
                routing_behavior_map=TunableMapping(
                    key_type=TunableReference(
                        manager=services.get_instance_manager(Types.OBJECT_STATE),
                        class_restrictions="ObjectStateValue",
                    ),
                    value_type=OptionalTunable(
                        tunable=TunableReference(
                            manager=services.get_instance_manager(Types.SNIPPET),
                            class_restrictions=("ObjectRoutingBehavior",),
                        ),
    ...
    def _inject_idle_component(self, obj, should_create_component=True):
        if self.idle_animation_map is not None:
            if (
                hasattr(obj, "_components")
                and hasattr(obj._components, "idle_component")
                and obj._components.idle_component is not None
            ):
                idle_component = obj._components.idle_component
                new_idle_component = clone_factory_wrapper_with_overrides(
                    idle_component,
                    idle_animation_map=merge_dict(
                        idle_component.idle_animation_map,
                        new_items=self.idle_animation_map,
                    ),
                )
        ...
        if self.routing_component is not None:
            if hasattr(obj, "_components") and hasattr(
                ...
                new_object_routing_component = clone_factory_wrapper_with_overrides(
                    ...
                    routing_behavior_map=merge_dict(
                        routing_component.object_routing_component.routing_behavior_map,
                        new_items=self.routing_component.routing_behavior_map,
                    ),
                )

                new_routing_component = clone_factory_wrapper_with_overrides(
                    routing_component,
                    object_routing_component=new_object_routing_component,
                )
                inject_dict(obj, "_components", routing_component=new_routing_component)
            elif should_create_component:
                object_routing_component = create_factory_wrapper(
                    ObjectRoutingComponent,
    ...
    def _inject_state_component(self, obj, should_create_component=True):
        if (
            hasattr(obj, "_components")
            and getattr(obj._components, "state", None) is not None
        ):
            ...
            if self.timed_state_triggers is not None:
                overrides.timed_state_triggers = merge_dict(
                    state_component.timed_state_triggers,
                    new_items=self.timed_state_triggers,
                )
            ...
            len(self.states)
            or len(self.state_triggers)
            or self.timed_state_triggers is not None
        ):
            state_component = create_factory_wrapper(
                TunableStateComponent,
                states=self.states,
                state_triggers=self.state_triggers,
                timed_state_triggers=self.timed_state_triggers,
            )
            inject_dict(obj, "_components", state=state_component)

    def _inject_proximity_component(self, obj):
        if (
            len(self.proximity_buffs) > 0
            and hasattr(obj, "_components")
            and getattr(obj._components, "proximity_component", None) is not None
        ):
            proximity_component = obj._components.proximity_component
            new_proximity_component = clone_factory_wrapper_with_overrides(
                proximity_component,
                buffs=merge_list(proximity_component.buffs, self.proximity_buffs),
            )
            inject_dict(obj, "_components", proximity_component=new_proximity_component)

    def _inject_carryable_component(self, obj):
        if (
            self.carryable_component is not None
            and hasattr(obj, "_components")
            and getattr(obj._components, "carryable", None) is not None
        ):
            carryable_component = obj._components.carryable
            new_carryable_component = clone_factory_wrapper_with_overrides(
                carryable_component,
                provided_affordances=merge_list(
                    carryable_component.provided_affordances,
                    self.carryable_component.provided_affordances,
                ),
            )
    ...
    def _inject_portal_component(self, obj):
        if (
            self.portal_component is not None
            and hasattr(obj, "_components")
            and getattr(obj._components, "portal", None) is not None
        ):
            ...
            # State Values
            state_values_which_disable_portals = merge_mapping_lists(
                portal_component.state_values_which_disable_portals,
                self.portal_component.state_values_which_disable_portals,
            )

            new_portal_component = clone_factory_wrapper_with_overrides(
                portal_component,
                _portal_data=portal_data,
                state_values_which_disable_portals=state_values_which_disable_portals,
            )
            inject_dict(obj, "_components", portal=new_portal_component)

    def _inject_inventory_item_component(self, obj):
        if (
            self.inventory_item_component is not None
            and hasattr(obj, "_components")
            and getattr(obj._components, "inventory_item", None) is not None
        ):
            inventory_item_component = obj._components.inventory_item
            new_inventory_item_component = clone_factory_wrapper_with_overrides(
                inventory_item_component,
                valid_inventory_types=merge_list(
                    inventory_item_component.valid_inventory_types,
                    self.inventory_item_component.valid_inventory_types,
                ),
            )
    ...
    def _inject_inventory_component(self, obj):
        if (
            self.inventory_component is not None
            and hasattr(obj, "_components")
            and getattr(obj._components, "inventory", None) is not None
        ):
            inventory_component = obj._components.inventory
            new_inventory_component = clone_factory_wrapper_with_overrides(
                inventory_component,
                starting_objects=merge_list(
                    inventory_component.starting_objects,
                    self.inventory_component.starting_objects,
                ),
            )
    ...
    def _inject_object_locking_component(self, obj, should_create_component=True):
        if (
            self.object_locking_component is not None
            and hasattr(obj, "_components")
            and getattr(obj._components, "object_locking_component", None) is not None
        ):
            object_locking_component = obj._components.object_locking_component
            new_object_locking_component = clone_factory_wrapper_with_overrides(
                object_locking_component,
                super_affordances=merge_list(
                    object_locking_component.super_affordances,
                    self.object_locking_component.super_affordances,
                ),
            )
            inject_dict(
                obj,
                "_components",
                object_locking_component=new_object_locking_component,
            )
        elif self.object_locking_component is not None and should_create_component:
            object_locking_component = create_factory_wrapper(
                ObjectLockingComponent,
    ...
    def _inject_object_relationship_component_override(self, obj):
        if self.object_relationship_component_override is not None:
            inject_dict(
                obj,
                "_components",
                object_relationships=self.object_relationship_component_override,
            )
    ...
    def _inject_routing_component(self, obj, should_create_component=True):
        if self.routing_component is not None:
            ...
            if routing_component is None and should_create_component:
                ...
                inject_dict(
                    routing_component,
                    "routing_behavior_map",
                    self.routing_component.routing_behavior_map,
                )
                    ...
                    if default_value is not None:
                        ...
                        if state not in state_component._states:
                            ...
                            if state_info.reset_to_default:
                                state_component._state_reset_values[state] = (
                                    default_value
                                )
                            if state_info.reset_on_load_if_time_passes:
                                state_component._state_reset_if_time_passes_values[
                                    state
                                ] = default_value
                            state_component._client_states[state] = (
                                state_info.client_states
                            )
                            if state_info.tested_states_on_add is not None:
                                state_component._tested_states_on_add[state] = (
                                    state_info.tested_states_on_add
                                )
                            if state_info.tested_states_post_load is not None:
                                state_component._tested_states_post_load[state] = (
                                    state_info.tested_states_post_load
                                )
                            if state_info.tested_states_on_location_changed is not None:
                                state_component._tested_states_on_location_changed[
                                    state
                                ] = state_info.tested_states_on_location_changed
                            if state_info.tested_states_on_reset is not None:
                                state_component._tested_states_on_reset[state] = (
                                    state_info.tested_states_on_reset
                                )
                            if state_info.tested_states_on_save is not None:
                                state_component._tested_states_on_save[state] = (
                                    state_info.tested_states_on_save
                                )
        ...
        if self.object_locking_component is not None:
            ...
            if locking_component is not None:
                add_affordances(
                    locking_component,
                    self.object_locking_component.super_affordances,
                    key="super_affordances",
                )
            elif should_create_component:
                locking_component = create_factory_wrapper(
                    ObjectLockingComponent,
                    super_affordances=self.object_locking_component.super_affordances,
                )
