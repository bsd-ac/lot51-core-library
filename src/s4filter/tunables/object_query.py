    ...
    def filter_objects_gen(self, obj_list=()):
        ...
        if len(_obj_list) == 0:
            ...
            num_choices = random.randint(
                min(len(_obj_list), self.limit.lower_bound),
                min(len(_obj_list), self.limit.upper_bound),
            )
...
class ObjectFilterAmountPerRoom(HasTunableSingletonFactory, AutoFactoryInit):
    ...
    def filter_objects_gen(self, obj_list=()):
        ...
        for room_id, objs in objs_by_room.items():
            num_choices = random.randint(
                min(len(objs), self.limit.lower_bound),
                min(len(objs), self.limit.upper_bound),
            )
...
class ObjectFilterVariant(TunableVariant):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
            first=ObjectFilterFirst.TunableFactory(),
            last=ObjectFilterLast.TunableFactory(),
            random_single_choice=ObjectFilterRandomSingleChoice.TunableFactory(),
            random_multiple_choice=ObjectFilterRandomMultipleChoice.TunableFactory(),
            amount_per_room=ObjectFilterAmountPerRoom.TunableFactory(),
            **kwargs,
        )
...
class ObjectSortVariant(TunableVariant):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
            closest=ObjectSortClosest.TunableFactory(),
            farthest=ObjectSortFarthest.TunableFactory(),
            weighted_random=ObjectSortWeightedRandom.TunableFactory(),
            random=ObjectSortRandom.TunableFactory(),
            **kwargs,
        )
    ...
    def _filter_exclude_tags_gen(self, obj_list):
        for obj in obj_list:
            if (
                ...
                and hasattr(obj, "has_any_tag")
                and obj.has_any_tag(self.exclude_tags)
            ):
                continue
            yield obj
    ...
    def _get_objects_gen(self, resolver=None):
        ...
        if inventory is not None:
            for obj in inventory:
                if (self.hidden and not inventory.is_object_hidden(obj)) or (
                    not self.hidden and inventory.is_object_hidden(obj)
                ):
                    continue
                yield obj
        ...
        if resolver is not None:
            ...
            def test_situation(situation):
                if (
                    ...
                    and situation.guid64 == self.situation.guid64
                ):
                    return True
...
class GetObjectsOnActiveLot(_GetObjectsBase):
    FACTORY_TUNABLES = {"allow_sims": Tunable(tunable_type=bool, default=False)}

    __slots__ = ("allow_sims",)

    def _get_objects_gen(self, resolver=None):
        for obj in services.object_manager().valid_objects():
            if obj.is_on_active_lot() and (
                not obj.is_sim or (obj.is_sim and self.allow_sims)
            ):
                yield obj
...
class GetObjectsByAffordance(_GetObjectsBase):
    ...
    def _get_objects_gen(self, resolver=None):
        for obj in services.object_manager().valid_objects():
            if (
                obj._super_affordances is not None
                and self.affordance in obj._super_affordances
            ):
                yield obj
...
class GetObjectsBySimInfo(_GetObjectsBase):
    ...
    def _get_objects_gen(self, resolver=None):
        for sim_info in services.sim_info_manager().get_all():
            ...
            sim = (
                services.object_manager().get(sim_info.id)
                if sim_info.is_baby
                else sim_info.get_sim_instance(allow_hidden_flags=allow_hidden_flags)
            )
            if sim is not None:
                yield sim
