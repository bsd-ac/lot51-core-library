...
class ServiceManager:
    ...
    def register_service(self, factory, init_critical=False, early_load=False):
        try:
            service = factory()
            if init_critical:
                self._init_critical_services[factory] = (
                    service,
                    early_load,
                )
            else:
                self._services[factory] = (
                    service,
                    early_load,
                )
            return service
            ...
            logger.exception(
                "[ServiceManager] failed registering service: {}".format(factory)
            )
    ...
    try:
        for key, service_tuple in service_manager.get_critical_services_gen():
            try:
                ...
                logger.exception(
                    "[_start_game_services] failed registering critical service: {}".format(
                        key
                    )
                )
        ...
        for key, service_tuple in service_manager.get_services_gen():
            try:
                ...
                logger.exception(
                    "[_start_game_services] failed registering service: {}".format(key)
                )
