...
def command_refresh_purchase_picker(
    purchase_picker: TunableInstanceParam(Types.SNIPPET), _connection=None,
):
    if purchase_picker is not None:
        stock_manager = purchase_picker.get_stock_manager()
        stock_manager._refresh_required = True
        output(
            f"Successfully refreshed picker stock: {purchase_picker.__name__}",
            _connection,
        )


@Command("purchase_picker.open", command_type=CommandType.Live)
def command_open_purchase_picker(
    purchase_picker: TunableInstanceParam(Types.SNIPPET), _connection=None,
):
    try:
        if purchase_picker is not None:
            ...
            logger.error(
                "[command][purchase_picker.open] Unable to find purchase picker snippet",
            )
    except:
        logger.exception(
            "[command][purchase_picker.open] Failed to open purchase picker",
        )


@Command("lot51_core.open_url", command_type=CommandType.Live)
def command_on_version_dialog_response(
    base_url: str = None, params: str = None, _connection=None,
):
...
def command_toggle_debug_logging(_connection=None):
    ...
    output(f"Logging Debug Mode: {mode}", _connection)
...
def command_print_config_debug(_connection=None):
    ...
    for config in instanced_configs:
        output(
            f"Name: {config.config_name}, Household Name: {config.household_name}",
            _connection,
        )
