    ...
    def __call__(self, subjects=(), affordance=None, **kwargs):
        ...
        if actor:
            if actor.get_mood() != self.mood:
                return TestResult(
                    False,
                    "Sim's mood does not match required mood for intensity test",
    ...
    def __call__(self, subjects=(), **kwargs):
        try:
            ...
            if actor and hasattr(actor, "Buffs"):
                ...
                total_weight = sum(
                    buff_entry.mood_weight
                    for buff_entry in buff_component._active_buffs.values()
                    if buff_entry.mood_type is active_mood
                )
