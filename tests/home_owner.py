...
class HomeOwnerWithRequirementsOnLot(
    ...
    FACTORY_TUNABLES = {
        ...
        "sim_info": SimInfoTest.TunableFactory(
            locked_args={"tooltip": None, "who": ParticipantType.Actor},
        ),
    }

    __slots__ = ("invert", "sim_info", "subject")
    ...
    def __call__(self, **kwargs):
            ...
            for sim in household.instanced_sims_gen():
                ...
                if resolver(self.sim_info):
                    if self.invert:
                        return TestResult(
                            False,
                            "Home owner on lot with requirements and is not the desired result",
                        )
        ...
        return TestResult(
            False,
            "Home owner not on lot with requirements and is not the desired result",
        )
