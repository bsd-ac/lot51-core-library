...
class CustomFoodRestrictionTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    HAS_RESTRICTIONS = 2
    FACTORY_TUNABLES = {
        "sim": TunableEnumEntry(
            description="The sim to check food restrictions for.",
            tunable_type=ParticipantTypeSingleSim,
            default=ParticipantTypeSingleSim.Actor,
        ),
        "object": TunableEnumEntry(
            description="The food object to check food restrictions against.",
    ...
    def recipe_has_restriction(self, tracker, recipe, restriction=None):
        ...
        if restriction is not None:
            if (
                restriction in recipe.food_restriction_ingredients
                and restriction in tracker._food_restriction_ingredients
            ):
                return True
            return False
        return any(
            ingredient in tracker._food_restriction_ingredients
            for ingredient in recipe.food_restriction_ingredients
        )
    ...
    def __call__(self, sim=None, object=None):
        ...
        if not (tracker and object.has_component(CRAFTING_COMPONENT)):
            ...
            if self.test == self.HAS_RESTRICTIONS:
                return TestResult(
                    False,
                    "Sim {} does not have a food restriction against {}",
                    sim,
                    object,
                    self.tooltip,
                )
        ...
        if self.test == self.NO_RESTRICTIONS and has_restriction:
            return TestResult(
                False,
                "Sim {} has a food restriction against {}",
                sim,
                object,
                self.tooltip,
            )
